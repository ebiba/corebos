...
class DocumentsUtils {
	...
	public static function modDocs_changeModuleVisibility($tabid, $status) {
		if ($status == 'module_disable') {
			Documents::disableDocumentsForModule($tabid);
		} else {
			Documents::enableDocumentsForModule($tabid);
		}
	}
	public static function modDocs_getModuleinfo() {
		global $adb;
		$query = $adb->pquery(
			"SELECT vtiger_notes_tabs.visible,vtiger_tab.name,vtiger_tab.tabid
				FROM vtiger_tab
				LEFT JOIN vtiger_notes_tabs ON vtiger_notes_tabs.tabid = vtiger_tab.tabid
